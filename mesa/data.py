...
def read_data(fname, header_start=4, remove_duplicates=True):
    """
    Function which takes in the summary.txt file written by MESA/GYRE and return
    an astropy table.
    ...
    colname_row = text[header_start+1]
    data_rows = text[header_start+2:-1]
...
class MesaModel(Table):
    """
    Table-like object for storing MESA .mod file data, extending astropy.table.Table object
    """
    def __init__(self, fname):
        ...
        for line in lines:
            ...
            if (line[0] == '!') or (len(items) == 0) or ('previous model' in line):
                continue

            # log setting for column data if not logged yet
            if ('setting' not in self.attr.keys()) and ('--' in items):
                setting = int(items[0])
                self.attr['setting'] = setting
                continue
            ...
            if np.in1d(items, list(model_attr_data_underscores.keys())).any():
                ...
                for ii, item in enumerate(items):
                    if item in model_attr_data_underscores.keys():
                        ...
                        self.attr[attr_name] = attr_entry
