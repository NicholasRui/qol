...
class MesaInlistProjectFile:
    ...
    """
    def __init__(self, mesa_version=config.mesa_version, LOGS_dir='LOGS/'):
        self.mesa_version = mesa_version

        self.inlist_controls = [] # controls
        self.prerequisites = [] # model files and other things which are required for this to work
    ...
    def save(self, fname=None):
        ...
        if fname = None, still return text but don't save any file
        """
...
class MesaWorkDirectory:
    ...
    """
    def __init__(self, mesa_version=config.mesa_version):
        self.mesa_version = mesa_version
        ...
        # - inlist pgstar

        # - save shell for running, according to template
        # - make plot which charts
        # - input to get history columns list

        self.history_columns_path = None
        self.profile_columns_path = None
    
    def copy_history_columns_list(self, fname):
        """
        Copy history_columns.list file from fname
        """
        if os.path.exists(fname):
            self.history_columns_path = fname
        else:
            raise ValueError(f'No path found: {fname}')
        
    def copy_profile_columns_list(self, fname):
        """
        Copy history_columns.list file from fname
        """
        if os.path.exists(fname):
            self.profile_columns_path = fname
        else:
            raise ValueError(f'No path found: {fname}')
    ...
    def check_and_sort(self):
        ...
        that each inlist will eventually have its prerequisites
        """
        ...
    ...
    def save_directory(path):
        ...
        # copy over history and profile columns files, if not None
