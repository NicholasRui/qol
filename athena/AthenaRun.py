...
class AthenaRun:
    ...
    def save_directory(self, grant_perms=False,
        ...
        bash_script.add_task(f"if ! cmp -s {os.path.join(run_path, 'configure.log')} $ATHENA_DIR/configure.log || ! [ -f $ATHENA_DIR/bin/athena ] || ! cmp -s {os.path.join(run_path, pgen_path_basename)} $ATHENA_DIR/src/pgen/{pgen_path_basename}; then")
